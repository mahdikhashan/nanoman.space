  ...
  return (
    <div className="relative md:inline-flex">
      <div 
        ...
        className={clsx(
          "md:hidden cursor-pointer transition-height transition-slowest ease fixed top-4 left-4 right-4 z-10 text-white bg-zinc-800 border-2 p-2 rounded-2xl",
          ...
          return (!link?.isDisabled &&
            (<div key={link.name} className={"p-2"}>
              <Link
                href={`/${link.slug}`}
                className={clsx(
                  "mb-2 px-3 text-sm font-semibold uppercase tracking-wider text-zinc-500",
                  { 'text-zinc-400': !isActive, 'text-orange-500': isActive },
                )}
                >
                  {link.name}
              </Link>
